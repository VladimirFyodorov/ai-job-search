...
⚠️ /status — это статус ПОИСКА РАБОТЫ (данные из Notion), НЕ системный статус. Не показывать задачи, Docker, cron — только данные по вакансиям и заявкам.

1. notify_start("status", sender=reply)
2. Вызвать `tools/status.py` через Python import:
```python
import sys, os
sys.path.insert(0, '/app')
ctx = int(os.environ.get('CLAUDE_CONTEXT_PCT', 0))
from tools.status import get_status
text = get_status(context_pct=ctx)
```
3. reply(text)
4. notify_done("status", "готово", sender=reply)

### Status Format

```
🤖 Hunter v2
  Запущен: сегодня в HH:MM / вчера в HH:MM
  Контекст: N%

📅 Дайджест: следующий в 09:00 (через {Nч Mm})

📊 Вакансии (Notion):
  Всего: N | Новых сегодня: N | Подходящих (score≥X): N | Новые (без оценки): N

📬 Заявки:
  Всего: N | Ожидают ответа: N | Интервью: N

🕒 Последняя активность:
  Поиск: X ч назад | Заявка: вчера / N дней назад

🔧 Сервисы:
  Telegram: ✅ | Notion: ✅/❌ | LaTeX: ✅/❌
```
