...
2. Collect stats: context_pct (from CLAUDE_CONTEXT_PCT env var) + tools/status.py data
3. Build formatted response (see Status Format below)
4. reply(formatted_response)
5. notify_done("status", "готово", sender=reply)

### Status Format

```
🤖 Hunter v2 · контекст {context_pct}%

📅 Дайджест: следующий в 09:00 (через {Nч Mm})

📊 Вакансии:
  Всего: N | Новых сегодня: N | Подходящих: N
